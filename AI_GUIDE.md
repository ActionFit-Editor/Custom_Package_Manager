...
- Current package version at generation time: `2.1.0`
...
- `Editor/Scripts/ActionFitSdkInstallProfile.cs` and `ActionFitSdkInstallProfileValidator.cs`: SDK profile schema versions 1 through 3, immutable source metadata, Unity Asset inventories, module graph, detection rules, and deterministic validation.
- `Editor/Scripts/ActionFitSdkUnityPackageArchive.cs`: read-only `.unitypackage` gzip/tar reader and archive-safety validator used before any AssetDatabase or project mutation.
...
- Repository metadata inspection must disable automatic redirects and manually follow only HTTPS redirects whose host remains exactly `api.github.com`, recreating the authenticated request at every hop. When the final GitHub `full_name` exactly matches the selected publish target, treat the catalog source as an already-completed repository transfer and do not require repository migration. Untrusted redirects and redirects resolving to a different repository must retain the normal failure or migration path.
...
- Profile schema version 1 declares identity/version, bridge package ID, vendor/license/support metadata, Unity/platform compatibility, allowed domains, immutable artifact/Git/registry sources, modules, ordered dependencies, scoped registries, and detection rules. Schema version 2 adds source `ResolutionPolicy`, matching `LatestResolver`, official `MetadataUrl`, and optional `VersionFamily` without changing schema-v1 behavior. Schema version 3 adds the `unitypackageArtifact` source kind with a complete `AssetInventory` (project-relative path, Unity GUID, per-file SHA-256, and file/folder kind) plus `PreservePaths` and `ExcludedPaths`, without changing schema-v1 or schema-v2 behavior. Version gates are per-feature: latest resolution requires `LatestResolutionSchemaVersion`, and `unitypackageArtifact` requires `UnityPackageSchemaVersion`, so raising the current version never invalidates an older profile. `unitypackageArtifact` sources must pin an exact immutable artifact and may not use a latest policy; only they may declare asset fields.
...
- Official artifact downloads require credential-free HTTPS on `AllowedDomains`, SHA-256 verification, and matching tarball `package/package.json` ID/version before any project mutation. Identity validation walks tar entries, skips every non-target payload without buffering it, and applies the 8 MiB bounded-read limit only to the returned `package/package.json` entry, so an official tarball that stores its metadata behind large vendor binaries stays valid while a truncated, mismatched, or oversized-metadata artifact still blocks. Git sources use full commits or exact SemVer tags. An optional `GitSubpath` must be a traversal-free relative path made of safe segments and is composed as `?path=<subpath>#<revision>`; source URLs themselves must remain query-free. Registry sources use exact SemVer.
- `unitypackageArtifact` sources are fully validated in memory before any project write: exact SHA-256, gzip/tar integrity, ustar headers, and pax extended headers. The reader rejects links, absolute or traversal paths, GNU long-name extensions, member depth other than `<guid>/<member>`, non-GUID directories, unexpected member names, duplicate members, duplicate or case-colliding asset paths, and an `asset.meta` whose GUID does not match its directory. It ignores macOS AppleDouble (`._`) resource forks, which official artifacts ship in large numbers, and allows a missing `asset.meta` only inside folder-shaped assets such as `.androidlib`, `.bundle`, and `.framework`, where Unity does not generate one. The extracted inventory must then match the declared profile inventory exactly; any undeclared, missing, GUID-drifted, or hash-drifted entry blocks execution.
- Unity Asset transactions run between the artifact and manifest phases as `AssetsCommitted`. Content and `.meta` are journaled as one entry so rollback restores both and never leaves an asset without its GUID. Existing `.meta` files are never overwritten, adopted and preserved entries are execution no-ops, and journal validation bounds every asset target, backup, and staging path to the project and transaction roots before replay.
- `ProjectSettings/ActionFitSdkProfiles.json` is ownership schema version 1. It records dependency values, registry scopes, cached artifacts, and Unity Assets created or explicitly adopted by a profile. Removal must preserve user-managed, changed, and shared entries.
- Unity Asset ownership records the project-relative path, GUID, content SHA-256, and one state of `created`, `adopted`, `shared`, `generated`, or `projectOwned`. Ownership binds to the asset GUID and content hash only. The `.meta` payload is deliberately not hashed because dependency resolvers rewrite importer labels in place, and treating that normalization as drift would block every adoption. Removal deletes only `created` entries, skips entries another profile still records, preserves a folder that still holds unmanaged content, and downgrades to preservation when owned content changed after apply.
