...
- Current package version at generation time: `1.1.81`
...
- `Editor/Scripts/ActionFitPackageRepositoryMigration.cs`: read-only source/target visibility and ref comparison plus separately approved, non-force Public-to-Private history migration and post-copy verification.
...
- AI bulk publishing must use `ActionFitPackageBulkPublishApi.PrepareAllChanged` before `ExecuteAll`. In-process callers should pass the returned `ApprovedPlan` object; execution skips duplicate package contract processes only when that object retains its non-serialized validation receipt, but still refreshes the catalog, recomputes file/version state, and rechecks GitHub in bounded parallel work before comparing the exact bulk plan and approval text. It must provide exact `ApprovedRepositoryCreationPackageIds` and `ApprovedRepositoryMigrationPackageIds` sets plus the separate migration approval text when returned. The Manager Console must use this same API rather than a separate bulk implementation.
- A catalog source URL that differs from a selected Private target must trigger read-only migration preflight. Inspect actual source/target visibility, default branches, and every branch/tag ref; reject an existing Public target, conflicting refs, missing source, current-version tag reuse, or README/AI guide URLs that still point only to the source. Never change, archive, delete, force-push, or prune the source. After explicit migration approval, create only the target when separately approved, push source branches/tags without force, align and verify the target default branch, then publish the current version and update catalog rows. Any failure before verification must leave catalog URLs unchanged and support safe retry.
...
- `Publish All Changed` uses snapshot-based publish requests so background tasks do not read Unity objects. It reuses approved contract results, rechecks mutable state, runs GitHub remote preflight and repository publishing with bounded parallelism, appends catalog rows by `upsertPackageVersions`, and keeps retryable catalog rows when append fails after successful repository publish.
- Public-to-Private relocation must finish its separately approved source/target ref copy and post-copy verification before package publication begins. The source remains unchanged, and catalog append remains after all repository work succeeds.
