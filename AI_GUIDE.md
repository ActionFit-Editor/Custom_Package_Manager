...
- Current package version at generation time: `1.1.95`
...
- Content bundle profiles must use HTTPS canonical Git URLs pinned to exact version tags. Preserve embedded, local/file, forked, branch, unparseable, newer canonical, shared, and user-modified package values according to the planner result.
...
- `ActionFitSdkInstallApi.Inspect` and `Plan` are read-only. Execution is available only through operation-specific `ApplyAsync`, `RepairAsync`, `UpdateAsync`, or `RemoveAsync` with the exact reviewed plan ID.
...
- Profile schema version 1 declares identity/version, bridge package ID, vendor/license/support metadata, Unity/platform compatibility, allowed domains, immutable artifact/Git/registry sources, modules, ordered dependencies, scoped registries, and detection rules.
...
- Plans bind the normalized profile snapshot, selected module closure, operation, original manifest/ownership hashes, desired content, and redacted changes. Revalidate every mutable snapshot immediately before execution.
