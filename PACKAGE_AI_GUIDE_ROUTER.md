...
- `Packages/com.actionfit.firebase-cli/AI_GUIDE.md` - ActionFit Firebase CLI owns prerequisite diagnosis, read-only Firebase config inspection, content-bound local initialization, emulator verification, approval-gated deployment orchestration, redacted receipts, and the template extension contract.
- `Packages/com.actionfit.firebase-cli_actionfit/AI_GUIDE.md` - Firebase CLI ActionFit bootstraps secret-free ActionFit and Stormborn profiles into the shared `_Data` settings asset while preserving project-owned IDs and existing values.
