...
- `Packages/com.actionfit.ai-contentpackage/AI_GUIDE.md` - AI Content Package Standard owns portable `AFCP-*` completion gates and deterministic read-only content-package auditing. Read before packaging or declaring a reusable content flow complete.
...
- `Packages/com.actionfit.ai-jira.core/AI_GUIDE.md` - AI Jira Core owns policy-neutral Jira configuration, credential-safe REST/JQL transport, ADF conversion, and issue/property primitives. Read when changing the facade transport boundary.
- `Packages/com.actionfit.ai-plan/AI_GUIDE.md` - AI Plan owns Jira-independent Plan documents, deterministic selection, guarded lifecycle transitions, execution handoff, results, and QA handoff data. Read when creating, selecting, executing, validating, or changing AI Plans.
- `Packages/com.actionfit.ai-plan.github/AI_GUIDE.md` - AI Plan GitHub owns fixed Plan-repository setup, normalized project mapping, personal queue branches, fast-forward synchronization, optimistic claims, and Git recovery evidence. Read when connecting or synchronizing Git-backed Plans.
- `Packages/com.actionfit.ai-plan.jira/AI_GUIDE.md` - AI Plan Jira owns assigned exact-todo discovery, selective source-linked Plan import without Jira writes, and optional three-field QA publishing. Read when importing Jira work into Plans or preparing Plan QA handoffs.
...
- `Packages/com.actionfit.ai-unitycli/AI_GUIDE.md` - AI UnityCLI routes official Unity lifecycle operations and running-Editor inspection/control through two approval-gated CLI surfaces.
- `Packages/com.actionfit.ai-workagent/AI_GUIDE.md` - AI WorkAgent defines portable pre-work task classification, high/xhigh reasoning-effort routing, agent-role boundaries, and risk-proportional verification. Read before non-trivial development work or when changing WorkAgent rules or skills.
...
- `Packages/com.actionfit.cat.fonts/AI_GUIDE.md` - ActionFit Cat Fonts owns Cat locale fallback assets, FontFallbackBinder policy, Dynamic TMP cleanup, glyph tooling, and tests. Read when changing fallback order, Resources behavior, locale refresh, glyph baselines, or font provenance.
...
- `Packages/com.actionfit.content.assets/AI_GUIDE.md` - ActionFit Content Assets owns the private central image ledger, rights evidence, canonical paths, and one-way UI dependency contract.
- `Packages/com.actionfit.cookie-cleanup/AI_GUIDE.md` - ActionFit Cookie Cleanup; read when changing or diagnosing this package.
- `Packages/com.actionfit.cookie-cleanup.ui/AI_GUIDE.md` - ActionFit Cookie Cleanup UI; read when changing or diagnosing this package.
...
- `Packages/com.actionfit.fonts.maplestory/AI_GUIDE.md` - ActionFit Maplestory Fonts owns canonical Maplestory TTF, Dynamic TMP assets, and preserved UI variants. Read when changing shared font ownership, GUID/reference migration, visual materials, provenance, or rights gates.
- `Packages/com.actionfit.force-update/AI_GUIDE.md` - ActionFit Force Update owns the scheduled version/build policy, the game-calendar day-boundary fetch rule, the daily-rollover gate contract, platform build-number providers, and the default blocking overlay.
...
- `Packages/com.actionfit.icecream-race.ui/AI_GUIDE.md` - ActionFit Ice Cream Race UI; read when changing or diagnosing this package.
...
- `Packages/com.actionfit.lava-rush/AI_GUIDE.md` - ActionFit Lava Rush owns the reusable engine; Cat Merge production follows the build-5.7.0 local source-parity decision.
...
- `Packages/com.actionfit.lava-rush.ui/AI_GUIDE.md` - ActionFit Lava Rush UI; read when changing or diagnosing this package.
...
- `Packages/com.actionfit.match-rival.ui/AI_GUIDE.md` - ActionFit Match Rival UI; read when changing or diagnosing this package.
...
- `Packages/com.actionfit.platform-account/AI_GUIDE.md` - ActionFit Platform Account owns SDK-neutral connection checks, silent and interactive ensure modes, single-flight authentication, cancellation, timeout, and project adapter boundaries.
...
- `Packages/com.actionfit.referencebinding/AI_GUIDE.md` - ReferenceBinding owns serialized-reference attributes, package-owned automatic Editor processing, explicit selection auto-wiring, deterministic no-write validation, structured diagnostics, and Player metadata boundaries. Read when changing those contracts, menus, tests, skills, metadata, or release flow.
- `Packages/com.actionfit.remote-config/AI_GUIDE.md` - ActionFit Remote Config owns SDK-neutral parameter reads, the activated snapshot cache, fetch completion and failure results, and last-successful-fetch reporting. It owns no fetch schedule.
- `Packages/com.actionfit.remote-config.firebase/AI_GUIDE.md` - ActionFit Remote Config Firebase owns the isolated Firebase Remote Config provider, its exact vendor version gate, fetch-instant normalization, and the missing-SDK diagnostic.
...
- `Packages/com.actionfit.sdk.google-play-review/AI_GUIDE.md` - ActionFit Google Play Review SDK owns the public source-only Google Play In-App Review install profile, official artifact checksums, shared External Dependency Manager reuse rules, and read-only Cat Merge migration inventory.
...
- `Packages/com.actionfit.ui.popup/AI_GUIDE.md` - UI Popup owns the compatible ViewController runtime, popup queue, lifecycle, Dim/sorting, scoped input lock, animation service boundary, and project adapter contract. Read when changing those APIs, migrated script identities, tests, or release behavior.
- `Packages/com.actionfit.ui.prefabs/AI_GUIDE.md` - UI Prefabs; read when changing or diagnosing this package.
